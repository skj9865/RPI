				      ...
				      struct snd_soc_dai *dai)
{
	...
	dev_warn(dev, " skj pcm512x_dai_startup_master start\n"); //  by skj
	...
	dev_warn(dev, " skj pcm512x_dai_startup_master end\n"); //  by skj
				     ...
				     struct snd_soc_dai *dai)
{
	...
	dev_warn(dev, " skj pcm512x_dai_startup_slave start\n"); //  by skj
	...
	dev_warn(dev, " skj pcm512x_dai_startup_slave end\n"); //  by skj
			       ...
			       struct snd_soc_dai *dai)
{
	...
	struct device *dev = dai->dev; // by skj 
	dev_warn(dev, " skj pcm512x_dai_startup start\n"); //  by skj
	pcm512x->fmt = SND_SOC_DAIFMT_CBS_CFS; // by skj
	...
	dev_warn(dev, " skj pcm512x_dai_startup end\n"); //  by skj
}
				  ...
				  enum snd_soc_bias_level level)
{
	...
	dev_warn(component->dev, " skj pcm512x_set_bias_level start\n"); //  by skj
	/*
	switch (level) {
...
*/
	dev_warn(component->dev, " skj pcm512x_set_bias_level end\n"); //  by skj
				      ...
				      unsigned long bclk_rate)
{
	 ...
	 */
	 ...
	 dev_warn(dev, " skj pcm512x_find_sck start\n"); //  by skj
	...
	dev_dbg(dev, "sck_rate %lu\n", sck_rate);
	 dev_warn(dev, " skj pcm512x_find_sck end\n"); //  by skj
	return sck_rate;
}
				  ...
				  unsigned long pll_rate)
{
	...
	unsigned long den;
	 ...
	 dev_warn(dev, " skj pcm512x_find_pll_coeff start\n"); //  by skj
...
fallback:
	...
	dev_warn(dev, " skj pcm512x_find_pll_coeff end\n"); //  by skj
					    ...
					    unsigned long pllin_rate)
{
	...
	struct device *dev = dai->dev; // by skj
	 dev_warn(dev, " skj pcm512x_pllin_dac_rate start\n"); //  by skj
	...
	for (dac_rate = rounddown(pcm512x_dac_max(pcm512x, 6144000), osr_rate);
	 ...
	 dev_warn(dev, " skj pcm512x_pllin_dac_rate end\n"); //  by skj
				...
				struct snd_pcm_hw_params *params)
{
	...
	int gpio;
	 ...
	 dev_warn(dev, " skj pcm512x_set_dividers start\n"); //  by skj
	...
	dev_dbg(component->dev, "1<<FSSP %d\n", 1 << fssp);
	 ...
	 dev_warn(dev, " skj pcm512x_set_dividers end\n"); //  by skj
			     ...
			     struct snd_soc_dai *dai)
{
	...
	int ret;
		...
		struct device *dev = dai->dev; // by skj
	 dev_warn(dev, " skj pcm512x_hw_params start\n"); //  by skj
...
skip_pll:
	 dev_warn(dev, " skj pcm512x_hw_params end\n"); //  by skj
	return 0;
}
...
static int pcm512x_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	int ret;
	 ...
	 dev_warn(component->dev, " skj pcm512x_set_fmt start\n"); //  by skj
...
*/
	pcm512x->fmt = fmt;
	 ...
	 dev_warn(component->dev, " skj pcm512x_set_fmt end\n"); //  by skj
	return 0;
}
...
static int pcm512x_set_bclk_ratio(struct snd_soc_dai *dai, unsigned int ratio)
{
	...
	struct pcm512x_priv *pcm512x = snd_soc_component_get_drvdata(component);
	 ...
	 dev_warn(component->dev, " skj pcm512x_set_bclk_ratio start\n"); //  by skj
	...
	pcm512x->bclk_ratio = ratio;
	 ...
	 dev_warn(component->dev, " skj pcm512x_set_bclk_ratio end\n"); //  by skj
...
static int pcm512x_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	unsigned int mute_det;
	 ...
	 dev_warn(component->dev, " skj pcm512x_mute start\n"); //  by skj
...
unlock:
	 dev_warn(component->dev, " skj pcm512x_mute end\n"); //  by skj
	mutex_unlock(&pcm512x->mutex);
*/
	return ret;
}
...
int pcm512x_probe(struct device *dev, struct regmap *regmap)
{
	...
	int i, ret;
	 ...
	 dev_warn(dev, " skj pcm512x_probe start\n"); //  by skj
...
*/
	...
	if (ret != 0) {
	 ...
	 dev_warn(dev, " skj pcm512x_probe end\n"); //  by skj
...
static int pcm512x_suspend(struct device *dev)
{
	...
	int ret;
	 ...
	 dev_warn(dev, " skj pcm512x_suspend start\n"); //  by skj
	...
	*/
	 ...
	 dev_warn(dev, " skj pcm512x_suspend end\n"); //  by skj
	return 0;
}
...
static int pcm512x_resume(struct device *dev)
{
	...
	int ret;
	 ...
	 dev_warn(dev, " skj pcm512x_resume start\n"); //  by skj
...
/* skj
	...
	*/
	 ...
	 dev_warn(dev, " skj pcm512x_resume end\n"); //  by skj
	return 0;
}
#endif
