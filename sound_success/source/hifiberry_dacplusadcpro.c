...
struct snd_soc_pcm_runtime *skj_rtd; 
static int snd_rpi_hifiberry_dacplusadcpro_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	dev_warn(rtd->dev, "skj DACplusADCpro init start");
	...
	if (ret < 0){
		dev_warn(rtd->dev, "skj DACplusADCpro init dac set fmt error");
		return ret;
	}
	...
	ret = snd_soc_dai_set_fmt(asoc_rtd_to_codec(rtd, 1),
		...
		| SND_SOC_DAIFMT_CBM_CFM);
	if (ret < 0){
		dev_warn(rtd->dev, "skj DACplusADCpro init adc set fmt error");
		return ret;
	}
	...
	if (ret < 0){
		dev_warn(rtd->dev, "skj DACplusADCpro init cpu set fmt error");
		return ret;
	}

	dev_warn(rtd->dev, "skj DACplusADCpro adc master reg set start 1");
	ret = snd_soc_component_write(adc, 0x00, 0x0); // page 0 select
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 1 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x20, 0x11); // master mode I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 2 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x0b, 0xCC); // pcm wordlength 16bit, audio interfaces I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 3 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x0d, 0x01); // 
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 4 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x26, 15); // 
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 5 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x27, 0x31); // 
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 6 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x01, 0x00); // pcm wordlength 16bit, audio interfaces I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 4 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x02, 0x00); // pcm wordlength 16bit, audio interfaces I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 5 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x06, 0x41); // pcm wordlength 16bit, audio interfaces I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 6 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x07, 0x41); // pcm wordlength 16bit, audio interfaces I2S
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 7 : %d\n",ret);
	dev_warn(rtd->dev, "skj DACplusADCpro adc master reg set end");
	...
	*/
	...
	dev_warn(rtd->dev, "skj DACplusADCpro adc master reg set start 2");
	ret = snd_soc_component_write(adc, 0x00, 0x0); // page 0 select
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 1 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x20, 0x11); // master mode I2S
	...
	dev_warn(rtd->dev, "skj DACplusADCpro init end");
	return 0;
}
	...
	struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_update_rate_den start");
	...
	rats_no_pll->num = CLK_48EN_RATE / 64; // by skj
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_update_rate_den end");
...
/*
	...
	*/
	...
	// by skj
	struct pcm186x_priv *pcm186x = snd_soc_component_get_drvdata(component);

	int ctype;

	ctype = HIFIBERRY_DACPRO_CLK48EN;
	clk_set_rate(pcm186x->sysclk, CLK_48EN_RATE);
	snd_rpi_hifiberry_dacplusadcpro_select_clk(component, ctype);
}
...
//	struct snd_soc_dai *dai = asoc_rtd_to_codec(rtd, 0);
	struct snd_soc_component *adc = asoc_rtd_to_codec(rtd, 1)->component; // by skj
	struct snd_soc_dai *dai = asoc_rtd_to_codec(rtd, 1); // by skj
	struct snd_soc_dai_driver *drv = dai->driver;
	const struct snd_soc_dai_ops *ops = drv->ops;

	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_hw_params start");
	
	if (snd_rpi_hifiberry_is_dacpro) {
		width = snd_pcm_format_physical_width(params_format(params));

//		snd_rpi_hifiberry_dacplusadcpro_set_sclk(dac,params_rate(params));
		snd_rpi_hifiberry_dacplusadcpro_set_sclk(adc,params_rate(params)); // by skj
	...
	if (ret)
		return ret;
	//ret = snd_soc_dai_set_bclk_ratio(asoc_rtd_to_codec(rtd, 0), channels * width); // by skj
	//ret = snd_soc_dai_set_bclk_ratio(asoc_rtd_to_codec(rtd, 1), channels * width);
	if (ret)
		return ret;
	if (snd_rpi_hifiberry_is_dacpro && ops->hw_params)
		ret = ops->hw_params(substream, params, dai);
	
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_hw_params end");
	return ret;
}
	...
	struct snd_pcm_substream *substream)
{
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_startup start");
//	if (leds_off)
//		return 0;

/////////////////////////////////////////////////////////////// by skj
	int ret = 0;
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_startup adc master reg set start 2");
	ret = snd_soc_component_write(adc, 0x00, 0x0); // page 0 select
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 1 : %d\n",ret);
	ret = snd_soc_component_write(adc, 0x20, 0x11); // master mode I2S
	...
	if (ret < 0)
		dev_warn(rtd->dev, "snd_soc_component_write Failed 7 : %d\n",ret);
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_startup adc master reg set end");
	///////////////////////////////////////////////////////////////////////// by skj
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_startup end");
	return 0;
}
	...
	struct snd_pcm_substream *substream)
{
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_shutdown start");
	...
	dev_warn(rtd->dev, "skj snd_rpi_hifiberry_dacplusadcpro_shutdown end");
}
...
static int snd_rpi_hifiberry_dacplusadcpro_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "skj hifiberry dacplusadcpro probe start");		
	...
	dev_info(&pdev->dev, "skj hifiberry dacplusadcpro probe 1");		
	if (ret) {
	...
	dev_info(&pdev->dev, "skj hifiberry dacplusadcpro probe 2");		
	...
	dev_info(&pdev->dev, "skj hifiberry dacplusadcpro probe 3");		
	...
	dev_info(&pdev->dev, "skj hifiberry dacplusadcpro probe end");			
	return ret;
}
