...
static int snd_rpi_hifiberry_dacplusadcpro_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	*/
	...
	snd_soc_component_write(adc, PCM186X_CLK_CTRL, 0x11); // master mode I2S
	snd_soc_component_write(adc, PCM186X_PCM_CFG, 0xCC); // pcm wordlength 16bit, audio interfaces I2S
...
/////// by skj
	...
	clk_set_rate(pcm186x->sclk, CLK_48EN_RATE);
...
//	ret = snd_soc_dai_set_bclk_ratio(asoc_rtd_to_codec(rtd, 0), channels * width);
	...
	ret = snd_soc_dai_set_bclk_ratio(asoc_rtd_to_codec(rtd, 1), channels * width); // by skj
	if (ret)
		return ret;
...
//	DAILINK_COMP_ARRAY(COMP_CODEC("pcm178x.1-004d", "pcm178x-hifi"),
...
static int snd_rpi_hifiberry_dacplusadcpro_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "skj dev info");		
